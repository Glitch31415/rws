...
If a server's listing in the activeservers list contains "triple-backticks required" in the notes section, then all data sent to the server needs to be between three backticks in order to be recognized. Send a test message to get the attention of the server. Example: "\```hi\```" (the contents of this message doesn't matter)
